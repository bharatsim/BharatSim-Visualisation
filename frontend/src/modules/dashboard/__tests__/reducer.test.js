...
import {autoSaveComplete, autoSaveError, autoSaveStarted, dashboardLoaded, updateDashboard} from "../actions";

describe("dashboard reducer", () => {
    const initialState = {autoSaveStatus: {}, dashboards: {}}
    const existingState = {autoSaveStatus: {"101": {saving: true}}, dashboards: {"101": {name: "existing"}}}
    ...
    it("should load dashboard", () => {
        const dashboard = {_id: "1", name: "dashboard"};
        const nextState = reducer(existingState, dashboardLoaded({dashboard}))
        expect(nextState).toEqual({
            ...existingState,
            dashboards: {...existingState.dashboards, [dashboard._id]: dashboard}
        })
    })
